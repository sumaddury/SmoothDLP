...
def test_factorize_naive_fully_factors_small_composite():
    factors, remainder = smooth.factorize_naive(360)
    assert remainder == 1
    assert sorted(factors) == [(2, 3), (3, 2), (5, 1)]


def test_factorize_naive_small_times_large_prime():
    mersenne61 = 2**61 - 1
    factors, remainder = smooth.factorize_naive(2 * mersenne61)
    assert remainder == 1
    assert sorted(factors) == [(2, 1), (mersenne61, 1)]


def test_factorize_naive_large_semiprime_with_no_small_factors():
    p, q = 1_099_511_628_211, 2_199_023_255_579
    assert smooth.is_prime(p) and smooth.is_prime(q)
    n = p * q
    factors, remainder = smooth.factorize_naive(n)
    assert factors == []
    assert remainder == n


def test_factorize_naive_randomized_reconstruction(rng):
    for _ in range(30):
        n = 1
        for _ in range(2 + rng.randrange(3)):
            candidate = random_bitlength(rng, 12 + rng.randrange(10))
...
def test_squfof_returns_nontrivial_factor_randomized(rng):
    for _ in range(20):
        p = _next_probable_prime(random_bitlength(rng, 20 + rng.randrange(15)), rng)
        q = _next_probable_prime(random_bitlength(rng, 20 + rng.randrange(15)), rng)
        if p == q:
            continue
        n = p * q
        if n >= 2**127:
